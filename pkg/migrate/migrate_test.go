...
func TestConvertConfig(t *testing.T) {
	...
	t.Run("unsupported provider warning", func(t *testing.T) {
		...
		if warnings[0] != "Provider 'deepseek' not supported in rdxclaw, skipping" {
	...
	t.Run("unsupported channel warning", func(t *testing.T) {
		...
		if warnings[0] != "Channel 'email' not supported in rdxclaw, skipping" {
			t.Errorf("unexpected warning: %s", warnings[0])
		}
	})
	...
	t.Run("agent defaults", func(t *testing.T) {
		...
		if cfg.Agents.Defaults.Workspace != "~/.rdxclaw/workspace" {
			t.Errorf("Workspace = %q, want %q", cfg.Agents.Defaults.Workspace, "~/.rdxclaw/workspace")
		}
	})
...
func TestRewriteWorkspacePath(t *testing.T) {
	tests := []struct {
		...
		{"default path", "~/.openclaw/workspace", "~/.rdxclaw/workspace"},
...
func TestRunDryRun(t *testing.T) {
	...
	rdxWs := filepath.Join(rdxclawHome, "workspace")
	if _, err := os.Stat(filepath.Join(rdxWs, "SOUL.md")); !os.IsNotExist(err) {
		t.Error("dry run should not create files")
	}
	if _, err := os.Stat(filepath.Join(rdxclawHome, "config.json")); !os.IsNotExist(err) {
...
func TestRunFullMigration(t *testing.T) {
	...
	soulData, err := os.ReadFile(filepath.Join(rdxWs, "SOUL.md"))
	if err != nil {
	...
	agentsData, err := os.ReadFile(filepath.Join(rdxWs, "AGENTS.md"))
	if err != nil {
	...
	memData, err := os.ReadFile(filepath.Join(rdxWs, "memory", "MEMORY.md"))
	if err != nil {
	...
	rdxConfig, err := config.LoadConfig(filepath.Join(rdxclawHome, "config.json"))
	if err != nil {
		t.Fatalf("loading rdxclaw config: %v", err)
	}
	if rdxConfig.Providers.Anthropic.APIKey != "sk-ant-migrate-test" {
		t.Errorf("Anthropic.APIKey = %q, want %q", rdxConfig.Providers.Anthropic.APIKey, "sk-ant-migrate-test")
	}
	if rdxConfig.Providers.OpenRouter.APIKey != "sk-or-migrate-test" {
		t.Errorf("OpenRouter.APIKey = %q, want %q", rdxConfig.Providers.OpenRouter.APIKey, "sk-or-migrate-test")
	}
	if !rdxConfig.Channels.Telegram.Enabled {
		t.Error("Telegram should be enabled")
	}
	if rdxConfig.Channels.Telegram.Token != "tg-migrate-test" {
		t.Errorf("Telegram.Token = %q, want %q", rdxConfig.Channels.Telegram.Token, "tg-migrate-test")
	}
