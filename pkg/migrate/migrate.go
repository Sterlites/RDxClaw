...
func Run(opts Options) (*Result, error) {
	...
	result := Execute(actions, openclawHome, rdxclawHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, rdxclawHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(rdxclawHome)
...
func Execute(actions []Action, openclawHome, rdxclawHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, rdxclawHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, rdxclawHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing rdxclaw config: %w", err)
		}
...
func resolverdxclawHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("RDXCLAW_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".rdxclaw"), nil
}
